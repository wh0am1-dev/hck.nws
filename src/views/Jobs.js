...
const Jobs = props => {
  ...
  const { fetching, jobs } = props
  useEffect(() => props.dispatch(fetchJobs()), []) // eslint-disable-line
  useEffect(() => {
    setShow(false)
    if (jobs && jobs.length) setTimeout(() => setShow(true), 50)
  }, [jobs])
...
export default connect(store => ({
  ...
  jobs: JSON.parse(JSON.stringify(store.jobs.items))
}))(Jobs)
